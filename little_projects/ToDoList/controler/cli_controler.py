...
def add_new_task(json_file_path):
    ...
    json.add_new_task(json_file_path, new_task)

def task_done(json_file_path):
    log.debug("task_done start")

def update_task(json_file_path):
...
def get_json_file_run():
    ...
    if json_select_index < len(json_default_list):
        ...
        elif specific_select == "All lists known":
            log.debug("All lists known selected")
            print("TODO : make this section")
            sys.exit(0)

def select_task_action(json_file_path):
...
def cli_controler_run():
    ...
    try:
        ...
        if not utils.check_json_file_origin(json_file_data):
